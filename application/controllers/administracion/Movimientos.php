		...
		if ($this->is_logged_in('reseller'))
		{
			...
			if ($this->form_validation->run() === false)
			{
				...
				if ($ultimo_movimientos_id = $this->movimiento_model->getIdUltimoMovimiento($default))
				{
					...
					if (!empty($this->input->get('id_factura'))) $default['id_factura'] = $this->factura_model->getFacturaDetalle($this->input->get('id_factura'))['id'];
				}
				...
				if (isset($data['detalle']['id_empresa'])) $data['facturas'] = $this->factura_model->comboFacturas(array('id_empresa'=>$data['detalle']['id_empresa']));
					...
					if (!empty($lines))
					{
						foreach ($lines as $line)
						{
							...
							if ($items[0] != 'EMPRESA' && isset($items[7]) && !$this->movimiento_model->verificarSiExiste(array(preg_replace("/[^0-9,.]/", "", $items[7]))))
							{
								...
								if (trim($items[12]) != false ) $data['observaciones'] .= ' (' . trim($items[12]) . ')';
