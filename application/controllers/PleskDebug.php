...
class PleskDebug extends MY_Controller {
...
*/
	...
	public function fix_recalcular($id)
	{
		if ($this->is_logged_in('admin'))
		{
			// models
			...
			if (!empty($plan) && isset($plan['id_servidor'])) {
				...
				if ($panel_type == 'plesk') {
					...
					// Obtener las estadísticas y actualizar manualmente
					$accountsummary = $this->plesk->accountsummary($plan['domain']);
					
					if (!empty($accountsummary)) {
						// Aplicar la actualización manualmente
						$result = $this->hosting_model->modificarAccount($plan['user'], $accountsummary);
						
						echo "<h2>Recálculo manual completado</h2>";
						echo "<p>Resultado: " . ($result ? "Éxito" : "Error") . "</p>";
						echo "<pre>" . print_r($accountsummary, true) . "</pre>";
						
						echo "<p><a href='" . base_url('hosting/detalle/' . $id) . "'>Volver a los detalles</a></p>";
					} else {
						echo "<h2>Error al obtener datos de la cuenta</h2>";
						echo "<pre>" . print_r($this->plesk->getLastRawData(), true) . "</pre>";
					}
				} else {
					echo "<p>Este servidor no es Plesk. Panel type: " . $panel_type . "</p>";
				}
