...
class Home extends MY_Controller
{
	...
	public function index()
	{
		...
		{
			// Redirigir al dashboard para resellers
	...
	public function revision()
	{
		if ($this->is_logged_in('reseller'))
		{
			...
			if (!isset($data['tickets']['nuevos']))
				$data['tickets']['abiertos'] = $this->ticket_model->getCantidadDeTickets(3);
			...
			if (!isset($data['servicios']['suspender']))
				$data['servicios']['activar'] = $this->servicio_model->getCantidadDeServicios(3);
			if (!isset($data['servicios']['activar']))
				$data['servicios']['suspendidos'] = $this->servicio_model->getCantidadDeServicios(1);
			...
			if (!isset($data['facturas']['imprimir']))
				$data['facturas']['error'] = $this->factura_model->getCantidadDeFacturas(7);
			...
			if (!isset($data['tareas']['vencidas']))
				$data['tareas']['vencidas'] = $this->tarea_model->getCantidadDeTareasVencidas();
			...
			if (!isset($data['comunicaciones']['error']))
