...
class Tickets extends REST_Controller {
    ...
    public function index_get($id = null)
    {
	    if ($id)
		{
	        ...
	        else
	        {
				...
				$data['agentes'] = $this->ticket_model->getTicketContactosAsociados($id);
				
				foreach ($this->ticket_model->getTicketItems($id) as $item)
				{
					$data['items'][$item['id']] = $item;
					$data['items'][$item['id']]['adjuntos'] = $this->ticket_model->getTicketItemAdjuntos($item['id']);;
				}
			}
		}
		else
		{
			...
			if ($this->input->get('id_contacto')) $parametros['id_contacto'] = $this->input->get('id_contacto');
	...
	public function responder_put()
	{
		...
		// set validation rules
		$this->form_validation->set_rules('id_ticket', 'Ticket', 'required|trim|integer');
		$this->form_validation->set_rules('id_contacto', 'Contacto', 'required|trim|integer');
		...
		else
		{
			...
			{
				$data['error'] = 'Ha habido un problema y no se pudo ingresar la respuesta, por favor intenta más tarde';
