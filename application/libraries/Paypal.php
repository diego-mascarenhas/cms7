...
class Paypal
{
    ...
    {
        $sanbox = $config['paypal_sandbox_mode'];
        $this->paypal_url = ($sanbox == true) ? 'https://www.sandbox.paypal.com/cgi-bin/webscr' : 'https://www.paypal.com/cgi-bin/webscr';
                
        $this->last_error = '';
        $this->ipn_response = '';

        $this->ipn_log_file = (isset($config['paypal_ipn_log_file'])) ? $config['paypal_ipn_log_file'] : 'logs/paypal.log';
        $this->ipn_log = (isset($config['paypal_ipn_log'])) ? $config['paypal_ipn_log'] : false; 

        
        // populate $fields array with a few default values.
        $businessEmail = $config['paypal_business_email'];
        $this->add_field('business', $businessEmail);
        $this->add_field('rm', '2');
        $this->add_field('cmd', '_xclick');

        $this->add_field('currency_code', $config['paypal_currency_code']);
    ...
    function button($value)
    {
        ...
        $this->submit_btn = '<input type="submit" name="pp_submit" value="Click here if you&#039;re not automatically redirected..."  />';
    }
    ...
    function paypal_auto_form()
    {
        ...
        echo ($this->auto_submit) ? '<body style="text-align:center;" onLoad="document.forms[\'paypal_auto_form\'].submit();">' . "\n" : '<body style="text-align:center;">' . "\n";
        if (!$this->auto_submit) echo '<p style="text-align:center;">Please wait, your order is being processed and you will be redirected to the paypal website.</p>' . "\n";
    ...
    function paypal_form($form_name='paypal_form')
    {
        ...
        if (!$this->auto_submit) $str .= '<p>'. $this->submit_btn . '</p>';
        $str .= "</form>\n";
