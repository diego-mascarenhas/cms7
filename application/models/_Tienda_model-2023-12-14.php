...
*/
	...
	{
		$sql = "SELECT tienda_medios_envios.*, tienda_rel_envios_tienda.tipo, tienda_rel_envios_tienda.descuento, tienda_rel_envios_tienda.recargo, tienda_rel_envios_tienda.orden";
...
/* 			$datos['subtotal'] = $variables['precio']*$variables['cantidad']; */
	...
	public function modificarTienda($valores)
	{
		...
		if($valores['privada'] == 1) { $datos['privada'] = 1; } else { $datos['privada'] = null; }
...
/* 		echo 'Cantidad: '.$array.'<br>'; */
    ...
    function relacionarEnvios($valores)
    {
		...
		foreach ($valores['relacionesenvios'] as $relacionado)
		{
			...
			if($valores['mmc3']) { $datos2['mmc'] = $valores['mmc3']; }
	...
	{
		$sql = "SELECT tienda_productos.id_categoria AS 'ID CATEGORIA', tienda_productos_categorias.categoria AS CATEGORIA, tienda_productos.titulo AS PRODUCTO, tienda_productos.contenido1 AS DESCRIPCION, tienda_productos.codigo AS CODIGO, tienda_productos.precio AS PRECIO, tienda_productos.precio_oferta AS 'PRECIO OFERTA', tienda_productos.precio_local AS 'PRECIO MENU', tienda_productos.precio_local_oferta AS 'PRECIO MENU OFERTA', 
					CASE
	...
	public function importarProducto($data, $tienda)
	{
		if( (isset($tienda)) && (!empty($data[0])) )
		{
			...
			if(!empty($data[4])) { $valores['codigo'] = $data[4]; } else { $valores['codigo'] = null; }
			if(!empty($data[5])) { $valores['precio'] = $data[5]; } else { $valores['precio'] = null; }
			if(!empty($data[6])) { $valores['precio_oferta'] = $data[6]; } else { $valores['precio_oferta'] = null; }
			if(!empty($data[7])) { $valores['precio_local'] = $data[7]; } else { $valores['precio_local'] = null; }
			if(!empty($data[8])) { $valores['precio_local_oferta'] = $data[8]; } else { $valores['precio_local_oferta'] = null; }
			if(!empty($data[9])) { $data[9] = strtolower($data[9]); if(convert_accented_characters($data[9]) == 'si') { $valores['galeria'] = 1; } else { $valores['galeria'] = 0;}  } else { $valores['galeria'] = 0; }
			if(!empty($data[10])) { $data[10] = strtolower($data[10]); if(convert_accented_characters($data[10]) == 'si') { $valores['destacado'] = 1; } else { $valores['destacado'] = 0;}  } else { $valores['destacado'] = 0; }
			if(!empty($data[11])) { $data[11] = strtolower($data[11]); if(convert_accented_characters($data[11]) == 'si') { $valores['estado'] = 3; } else { $valores['estado'] = 1; }  } else { $valores['estado'] = 1; }
