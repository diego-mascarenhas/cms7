...
class Media_model extends CI_Model {	
		...
		if($query->num_rows()) 
		{
            ...
            $file_ext = strtolower(end($tmp));
						...
						$data['files'][] = array(
								...
								'size' 		  	=> 	$this->format_bytes($md->file_size),
								'fecha_alta'	=>  $md->fecha_alta
							);
					}
				}
	...
	public function format_bytes($kbs)
  {
	...
	public function getMediaDetalle($id, $parametros = null)
	{
		if ($parametros['modo'] == 'raw')
		{
			$sql = "
					...
					FROM media_v1 AS media
				";
		}
		else
		{
			$sql = "
					SELECT media.*, MD5(CONCAT(media.id, media.user_id)) AS uid, trim(CONCAT(contactos.nombre, ' ', IFNULL(contactos.apellido, ''))) AS contacto
	...
	{
		$sql = "
				SELECT CONCAT(media.user_id, media.file_path, media.file_name) AS media
					
				FROM media_v1 AS media
			
				WHERE MD5(CONCAT(media.id, media.user_id)) = ?
			";
	...
	{
		$sql = "
				SELECT media.*, MD5(CONCAT(media.id, media.user_id)) AS uid
