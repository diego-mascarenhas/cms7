...
*/
	...
	{
		$sql = "SELECT tienda_medios_envios.*, tienda_rel_envios_tienda.tipo, tienda_rel_envios_tienda.descuento, tienda_rel_envios_tienda.recargo, tienda_rel_envios_tienda.observaciones, tienda_rel_envios_tienda.mmc, tienda_rel_envios_tienda.orden";
...
*/
	...
	{
		$sql = "SELECT tienda_pedidos_items.id, tienda_pedidos_items.id_producto, tienda_pedidos_items.detalle, tienda_pedidos_items.cantidad, tienda_pedidos_items.subtotal, tienda_pedidos_items.username_alta, tienda_productos.precio, tienda_productos.precio_local, tienda_productos.titulo, tienda_productos.codigo, tienda_productos.contenido1, tienda_productos.contenido2, tienda_productos.imagen";
...
/* 			$datos['subtotal'] = $variables['precio']*$variables['cantidad']; */
	...
	public function finalizarPedido($variables)
	{
		...
		//VERIFICO SI ES TIENDA BRULER PARA INGRESAR DATA EN SUS PEDIDOS
/*
		$sql_tienda = "SELECT bruler_id, id";
		$sql_tienda .= " FROM tienda_configuracion";
		$sql_tienda .= " WHERE titulo = "'".$variables['tienda']."'";
		$sql_tienda .= " AND estado = 3";
		$query = $this->db->query($sql_tienda);
		$res_tienda = $query->row_array();
		if($res_tienda['bruler_id'] > 0)
		{
		    $bruler_pedido = $this->finalizarPedidoBruler($res_tienda['id'], $variables['pedido']);
		}
...
*/
	...
	public function finalizarPedidoBruler($tienda, $pedido)
	{
		//SELECCIONO PEDIDOS EN ESTADO DIFERENTE A 1 (en curso)
		$sql = "SELECT tienda_pedidos.id, tienda_pedidos.id_sucursal, tienda_pedidos.id_tienda_origen, tienda_pedidos.numero_mesa, tienda_pedidos.nombre, tienda_pedidos.celular, tienda_pedidos.email, tienda_pedidos.domicilio, tienda_pedidos.observaciones, tienda_pedidos.subtotal, tienda_pedidos.descuento, tienda_pedidos.descuento_medios_envio, tienda_pedidos.descuento_items, tienda_pedidos.envio, tienda_pedidos.total, tienda_medios_envios.medio_envio, tienda_formas_pago.forma_pago, tienda_sucursales.titulo as nombre_sucursal, tienda_sucursales.email as tienda_email, tienda_sucursales.recibir_email, tienda_sucursales.celular as whatsapp, tienda_monedas.simbolo";
		$sql .= " FROM tienda_pedidos";
		$sql .= " LEFT JOIN tienda_medios_envios ON tienda_medios_envios.id = tienda_pedidos.id_medio_envio";
		$sql .= " LEFT JOIN tienda_formas_pago ON tienda_formas_pago.id = tienda_pedidos.id_forma_pago";
		$sql .= " LEFT JOIN tienda_sucursales ON tienda_sucursales.id = tienda_pedidos.id_sucursal";
		$sql .= " LEFT JOIN tienda_monedas ON tienda_monedas.id = tienda_sucursales.pais";
		$sql .= " WHERE tienda_pedidos.id = $pedido";
		$sql .= " AND tienda_pedidos.estado > 1";
		$query = $this->db->query($sql);
		$res = $query->row_array();
		...
		if($res['id'])
		{
			$data_pedido = json_encode($res);

			//VERIFICO SI HAY PEDIDO CARGADO EN BRULER PEDIDOS
			$sql = "SELECT *";
			$sql .= " FROM bruler_pedidos.*";
			$sql .= " WHERE id_pedido = $pedido";
			$query = $this->db->query($sql);
			$res_pedido = $query->row_array();

			if($res_pedido['id'])
			{
				$datos['data'] = $data_pedido;
				$datos['updated_at'] = unix_to_human(now(), true, 'eu');
				$where = "id_pedido = $pedido";
			    $update = $this->db->update('bruler_pedidos', $datos, $where);
			}
			else
			{
				$datos['type'] = 'orders';
				$datos['bruler_id_tienda'] = $tienda;
				$datos['id_pedido'] = $pedido;
				$datos['data'] = $data_pedido;
				$datos['status'] = 1;
				$datos['created_at'] = unix_to_human(now(), true, 'eu');
			    $insert = $this->db->insert('bruler_pedidos', $datos);
			}
