...
class Categorias_model extends CI_Model {
	...
	public function getCategorias($parametros = null)
	{
		$sql = "SELECT id, categoria, padre, orden, estado,
				(SELECT categoria FROM con_elearning_categorias AS padre WHERE padre.id = con_elearning_categorias.padre) AS padre_nombre";
		$sql .= " FROM con_elearning_categorias";
		$sql .= " WHERE grupo = ?";
	...
	{
		$sql = "SELECT id, categoria FROM con_elearning_categorias";
		...
		{
			$sql .= " AND con_elearning_categorias.padre >= 0";
		}
	...
	{
		$sql = "SELECT id, categoria, uri, imagen, padre, orden, estado, idioma, seccion, fecha_alta,
				(SELECT categoria FROM con_elearning_categorias AS padre WHERE padre.id = con_elearning_categorias.padre) AS padre_nombre,
				CASE
					WHEN estado = 1 THEN 'Inactivo'
					WHEN estado = 2 THEN 'Activo'
					END AS estado_nombre
				FROM con_elearning_categorias";
	...
	public function ingresarCategoria($valores)
	{
		...
		$sql1 = "SELECT id, orden FROM con_elearning_categorias";
		$sql1 .= " WHERE grupo = ?";
		$sql1 .= " AND id_empresa = ?";
		$sql1 .= " AND padre = ?";
		...
		$insert = $this->db->insert('con_elearning_categorias', $data);
	...
	public function modificarCategoria($valores)
	{
		...
		$where = "id = ".$valores['id'];
		$res = $this->db->update('con_elearning_categorias', $data, $where);
		return (!empty($res)) ? $res : null;
	}
	...
	public function duplicarCategoria($id)
	{
		$sql = "SELECT *";
		$sql .= " FROM con_elearning_categorias";
		$sql .= " WHERE con_elearning_categorias.estado > 0";
	...
	public function ordenarCategorias($items)
	{
		for ($i=0; $i<count($items); ++$i)
		{
		    ...
		    $this->db->update('con_elearning_categorias', $data, array('id'=>$items[$i]));
	...
	public function publicarCategoria($valores)
	{
		...
		$where = "id = ".$valores['id'];
		$update = $this->db->update('con_elearning_categorias', $data, $where);
		...
		$sql = "SELECT id, estado FROM con_elearning_categorias WHERE padre = ?";
		...
		foreach($hijos as $hijo)
		{
			...
			$update = $this->db->update('con_elearning_categorias', $data, $wherehijo);
