...
class Informacion_model extends CI_Model {
	...
	{
		$sql = "SELECT con_contenidos.id_con_secciones, con_contenidos.imagen_adicional, con_contenidos.color, con_contenido_items.id_contenido, con_contenido_items.url, con_contenido_items.titulo, con_contenido_items.subtitulo, con_contenido_items.texto_adicional, con_contenidos.fecha_alta, con_contenido_items.contenido1, con_contenido_items.contenido2, con_contenido_items.seo_titulo, con_contenido_items.seo_keywords, con_contenido_items.seo_descripcion, con_contenido_items.archivo1, media.id as id_media, (SELECT media_thumbs.archivo FROM media_thumbs WHERE media_thumbs.referencia = media.id AND media_thumbs.id_tipo = 14 LIMIT 1) AS imagen";
		$sql .= " FROM con_contenido_items";
		$sql .= " LEFT JOIN con_contenidos ON con_contenidos.id = con_contenido_items.id_contenido";
		$sql .= " LEFT JOIN con_rel_contenidos_media ON con_rel_contenidos_media.id_contenido = con_contenidos.id";
		$sql .= " LEFT JOIN media ON media.id = con_rel_contenidos_media.id_media";
		...
		{			
			...
			foreach($idiomas as $idioma)
			{
				...
				if($valores['titulo_'.$extension])
				{
					...
					if(isset($valores['seo_keywords_'.$extension])) { $item['seo_keywords'] = $valores['seo_keywords'.$extension]; } else { $item['seo_keywords'] = NULL;}
